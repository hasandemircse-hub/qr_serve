...
| Tarih | Özet | Modül |
|-------|------|--------|
| 2026-05-31 | **Fool-proof denetimi + sağlamlaştırma planı:** 5 paralel kod incelemesi ile ~30 KRİTİK + ~30 ÖNEMLİ dayanıklılık eksikliği tespit edildi (sipariş idempotency, optimistic lock exception handler, BaseEntity @PreUpdate LWW ping-pong, sync_outbox FAILED/DEAD eksikliği, WS handshake JWT yokluğu, Flutter reconnect/timeout/401 handler yok, Actuator+healthcheck yok, print queue in-memory, backup script yok, logback yok, vb.). Sonuçlar 15 atomik maddeye dönüştürüldü (F1–F15), 4 sprint'e bölündü (S0 mali güvenlik, S1 sync+WS, S2 UX, S3 ops). Pilot için ŞART. Detay/takip: [QUICKSERVE_FOOLPROOF_PLAN.md](./QUICKSERVE_FOOLPROOF_PLAN.md); roadmap'e FAZ 1.9 olarak eklendi. | docs |
| 2026-05-26 | **Self-contained Edge bundle (FAZ 3.10):** Edge frontend artık `deploy/edge/Dockerfile.caddy` içinde (multi-stage: `ghcr.io/cirruslabs/flutter:stable` → `caddy:2-alpine`) build edilip imaja gömülüyor. `deploy/edge/docker-compose.yml` her servis için `image: quickserve/edge[-caddy]:${EDGE_TAG:-local}` + `build:` hibrit (dev'de Mac'te build, üretimde bundle load). `deploy/scripts/build-edge-images.sh` `docker buildx` ile cross-platform imaj üretip **tek self-contained bundle** (≈190 MB `quickserve-edge-bundle-<tag>.tar.gz` — içinde `images/edge.tar`, `images/edge-caddy.tar`, `deploy/{docker-compose.yml,Caddyfile,.env.example}`, `install.sh`, `VERSION`) + `.sha256` çıkartır. Edge cihazında akış: `tar -xzf bundle.tar.gz && sudo bash install.sh [--start]` — `git clone` YOK, `docker build` YOK, Flutter/Maven YOK. `install.sh` `.env` otomasyonu da yapıyor (yoksa şablondan kopyala, varsa `EDGE_TAG`'i bundle ile senkronla). Eski `build-edge-frontend.sh` DEPRECATED. | deploy, edge_frontend, docs |
| 2026-05-26 | **Edge↔Cloud sync için shared-secret auth (FAZ 1.2):** `/api/v1/sync/**` artık `X-QuickServe-Sync-Key` header'ı ister. `common/auth/SyncSharedSecretFilter` Cloud security chain'inin başına yerleşti; eşleşme sabit zamanlı (`MessageDigest.isEqual`, timing-attack güvenli). Cloud env'inde `QUICKSERVE_SYNC_SHARED_SECRET` doluysa **zorunlu**, boşsa permissive + uyarı log'u (mevcut Edge'leri kırmadan cutover olanağı). Edge `RestClient` aynı env'i `QuickserveProperties.cloud.syncSharedSecret` üzerinden default header olarak ekler. `.env.example` ve docker-compose her iki tarafta passthrough; üretmek için `openssl rand -base64 48`. | common, cloud, edge, deploy, docs |
| 2026-05-26 | **Cloud→Edge gerçek sağlık testi (FAZ 1.3):** `POST /api/v1/admin/restaurants/{id}/edge-health-check` heartbeat'ten bağımsız olarak Cloud'un Edge'in public URL'ine gerçekten ulaşıp ulaşamadığını test eder; cevap içinde `edgeId`/`restaurantId` Cloud'un beklediği değerlerle çapraz doğrulanır (tunnel hijack / kimlik karışıklığı tespiti). `EdgeHealthCheckService` + `EdgeHealthCheckResult` (HTTP status, cevap süresi ms, hata kodu: `NO_PUBLIC_URL`, `HTTP_5xx`, `TIMEOUT`, `DNS_FAIL`, `EDGE_ID_MISMATCH`, ...). Süperadmin dashboard'a kalp ikonu butonu + sonuç dialog'u eklendi. | cloud, cloud_frontend, docs |
