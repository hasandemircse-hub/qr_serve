...
@Component
public class LayoutSessionRegistry {

	private static final Logger log = LoggerFactory.getLogger(LayoutSessionRegistry.class);
	...
	public void broadcast(UUID restaurantId, String json) {
		...
		for (WebSocketSession session : Set.copyOf(set)) {
			...
			try {
				...
				log.warn("WebSocket send failed, closing session {}: {}", session.getId(), ex.getMessage());
				try {
					session.close();
