...
@Component
public class WaiterPushSessionRegistry {

	private static final Logger log = LoggerFactory.getLogger(WaiterPushSessionRegistry.class);
	...
	public void broadcast(UUID restaurantId, String json) {
		...
		for (WebSocketSession session : Set.copyOf(set)) {
			...
			try {
				...
				log.warn("Waiter WS send failed: {}", ex.getMessage());
				try {
					session.close();
