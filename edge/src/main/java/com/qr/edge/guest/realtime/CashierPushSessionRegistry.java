...
@Component
public class CashierPushSessionRegistry {

	private static final Logger log = LoggerFactory.getLogger(CashierPushSessionRegistry.class);
	...
	public void broadcast(UUID restaurantId, String json) {
		...
		for (WebSocketSession session : Set.copyOf(set)) {
			...
			try {
				...
				log.warn("Cashier WS send failed: {}", ex.getMessage());
				try {
					session.close();
